...
class DecoderRNN(BaseRNN):
    ...
    def __init__(self,
        ...
        if use_attention:
            self.dec_attention = Attention(self.hidden_size)
            self.enc_attention = Attention(self.hidden_size)
    ...
    def forward_step(self, input_var, hidden, encoder_outputs, function):
        ...
        self.dec_outputs.append(output)
        attn = None
        if self.use_attention:
            dec_outputs = torch.cat(self.dec_outputs, dim=1)
            output, d_att = self.dec_attention.forward(output, dec_outputs)
            self.length_info.push(input_var)
            context = self.length_info.extract_context(encoder_outputs)
            output, attn = self.enc_attention.forward(output, context)
