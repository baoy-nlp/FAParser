...
class RNNStack(nn.Module):
    def __init__(self, rnn_cell, input_size, hidden_size, n_layers, dropout_p):
        super(RNNStack, self).__init__()
        self.push_rnn = rnn_cell(input_size, hidden_size, n_layers, batch_first=True, dropout=dropout_p)
        self.pop_rnn = rnn_cell(input_size, hidden_size, n_layers, batch_first=True, dropout=dropout_p)
    ...
    def process_hidden(self, push_hidden, pop_hidden, is_pop):
        ...
        if isinstance(_push, tuple):
            ...
            hiddens = tuple(
                [h[range(self.batch_size), is_pop, :].view(self.batch_size, -1, self.hidden_size).transpose(1, 0).contiguous() for h in pre_hid])
        else:
