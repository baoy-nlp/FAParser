...
if GlobalNames.use_fnn_attetnion:
...
class DecoderRNN(BaseRNN):
    ...
    def forward_step(self, input_var, hidden, encoder_outputs, function):
        ...
        grammar_input = self.input_dropout(self.grammar_embed(input_var))
        ...
        output = torch.cat((output, embedded, grammar_context), dim=-1)
        predicted_softmax = function(self.out(output.contiguous().view(-1, self.logit_size)), dim=-1).view(batch_size, output_size, -1)
        ...
        if use_teacher_forcing:
            ...
            for di in range(max_length):
                decoder_output, decoder_hidden, step_attn = self.forward_step(decoder_input, decoder_hidden, encoder_outputs,
                                                                              function=reflect)
                step_output = decoder_output.squeeze(1)
                symbols = decode_with_constrain(di, step_output, step_attn, mask=self.decoder_stack.mask)
