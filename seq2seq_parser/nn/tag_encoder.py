...
class EncoderRNN(BaseRNN):
    ...
    def __init__(self, src_vocab, tag_vocab, src_dim, tag_dim, max_len, hidden_size,
                 input_dropout_p=0, dropout_p=0,
                 n_layers=1, bidirectional=False, rnn_cell='gru', variable_lengths=False,
                 embedding=None, update_embedding=True):
        input_size = src_dim + tag_dim
        super(EncoderRNN, self).__init__(src_vocab, max_len, src_dim, hidden_size,
                                         input_dropout_p, dropout_p, n_layers, rnn_cell)
        self.tag_embedding = nn.Embedding(tag_vocab, tag_dim)
    ...
    def forward(self, input_var, input_lengths=None):
        ...
        src_var = input_var[0]
        tag_var = input_var[1]
        src_embed = self.embedding(src_var)
        tag_embed = self.tag_embedding(tag_var)
        embedded = torch.cat([src_embed, tag_embed], dim=-1)
