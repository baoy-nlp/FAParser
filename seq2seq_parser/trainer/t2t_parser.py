...
from seq2seq_parser.models import *
...
from seq2seq_parser.utils.data_io import ZipDatasets, TextDataset, DataIterator
...
def train(FLAGS):
    ...
    if model_cls not in seq2seq_parser.models.__all__:
        raise ValueError(
            "Invalid model class \'{}\' provided. Only {} are supported now.".format(
                model_cls, seq2seq_parser.models.__all__))
    ...
    for eidx in range(training_configs['max_epochs']):
        ...
        for batch in training_iter:
            ...
            if (np.mod(uidx, training_configs['bleu_valid_freq']) == 0 and uidx > training_configs['bleu_valid_warmup']) \
                ...
                valid_acc = metric_validation(uidx=uidx,
                                              valid_iterator=valid_iterator,
                ...
                model_collections.add_to_collection(key="history_bleus", value=valid_acc)

                best_valid_acc = float(np.array(model_collections.get_collection("history_bleus")).max())

                summary_writer.add_scalar("bleu", valid_acc, uidx)
                summary_writer.add_scalar("best_bleu", best_valid_acc, uidx)
