...
class PTBCreator(object):
    ...
    def __init__(self,
        ...
        train_filepath = os.path.join(data_path, "train.clean")
        valid_filepath = os.path.join(data_path, "dev.clean")
        test_filepath = os.path.join(data_path, "test.clean")

        assert process_path is not None
        dict_filepath = os.path.join(process_path, 'dict.pkl')
        data_filepath = os.path.join(process_path, 'parsed.pkl')
        embed_filepath = os.path.join(process_path, "ptb_glove.npy")  # '../data/ptb/ptb_glove.npy'
        ...
        if glove_path is not None:
            ...
            self.wordembed_matrix = embeddingstd * numpy.random.randn(maxvocabsize, wordembed_dim).astype('float32')
            key_error = 0
            for key, value in enumerate(self.dictionary.idx2word):
                try:
                    ...
                    key_error += 1
            print("Total vocab size: %d, tokens not found in glove: %d" % (maxvocabsize, key_error))
            del wv
...
if __name__ == '__main__':
    ...
    PTBCreator(data_path=sys.argv[1], process_path=sys.argv[2], glove_path=sys.argv[3] if len(sys.argv) > 3 else None)
