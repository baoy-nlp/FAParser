...
from distance_parser.dataloader import PTBLoader
from distance_parser.helpers import *
from distance_parser.loss import *
from distance_parser.distance_parser import DistanceModel
...
if __name__ == '__main__':
    ...
    model = DistanceModel(vocab_size=args.vocab_size,
    ...
    train_idxs, train_tags, train_stags, train_arcs, train_distances, train_sents, train_trees = ptb_parsed.batchify('train', args.bthsz)
    if args.devfre == -1:
    ...
    for epoch in range(start_epoch, start_epoch + args.epc):
        ...
        for ibatch, (idx, tag, stag, arc, dst) in \
            ...
            optimizer.zero_grad()
            pred_dst, pred_arc, pred_tag = model.forward(idx, stag, mask)
