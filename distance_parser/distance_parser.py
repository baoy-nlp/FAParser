...
from distance_parser.embed_regularize import embedded_dropout
from distance_parser.weight_drop import WeightDrop
...
class DistanceModel(nn.Module):
    def __init__(self,
        ...
        if wordembed is not None:
            self.word_embed.weight.data = torch.FloatTensor(wordembed)

        self.tag_embed = nn.Embedding(stag_size, embed_size)
    ...
    def forward(self, words, stag, mask):
        ...
        emb_stags = embedded_dropout(self.tag_embed, stag, dropout=self.dropoute if self.training else 0)
