...
from FAParser.modules.tensor_ops import get_tensor
...
class TopKDecoder(nn.Module):
    ...
    def __init__(self, decoder_rnn, k):
        ...
        self.V = self.rnn.vocab_size
        self.SOS = self.rnn.sos_id
        self.EOS = self.rnn.eos_id

    def beam_search(self, encoder_hidden, encoder_outputs, decode_max_time_step, beam_size=5):
        self.k = beam_size

        decoder_outputs, decoder_hidden, metadata = self.forward(
            inputs=None,
            encoder_hidden=encoder_hidden,
            encoder_outputs=encoder_outputs,
            teacher_forcing_ratio=0.0,
            dmts=decode_max_time_step
        )
        return metadata

    def forward(self, inputs=None, encoder_hidden=None, encoder_outputs=None, function=F.log_softmax,
                teacher_forcing_ratio=0.0, retain_output_probs=True, dmts=None):
        """
        Forward rnn for MAX_LENGTH steps.  Look at :func:`seq2seq.models.DecoderRNN.DecoderRNN.forward_rnn` for details.
        """

        inputs, batch_size, max_length = self.rnn.valid_args(inputs, encoder_hidden, encoder_outputs,
                                                             function, teacher_forcing_ratio)

        if teacher_forcing_ratio <= 0.0 and dmts is not None:
            max_length = dmts
        self.pos_index = Variable(get_long_tensor(range(batch_size)) * self.k).view(-1, 1)

        # Inflate the initial hidden states to be of size: b*k x h
        encoder_hidden = self.rnn.init_state(encoder_hidden)
        ...
        sequence_scores = get_tensor(torch.Tensor(batch_size * self.k, 1).data)
        sequence_scores.fill_(-float('Inf'))
        sequence_scores.index_fill_(0, get_long_tensor([i * self.k for i in range(0, batch_size)]), 0.0)
        sequence_scores = Variable(sequence_scores)

        # Initialize the input vector
        input_var = Variable(torch.transpose(get_long_tensor([[self.SOS] * batch_size * self.k]), 0, 1))
    ...
    def _backtrack(self, nw_output, nw_hidden, predecessors, symbols, scores, b, hidden_size):
        ...
        ret_l = [[max_t] * self.k for _ in range(b)]  # Placeholder for lengths of top-k sequences
        # Similar to `h_n`
        ...
        while t >= 0:
            ...
            if eos_indices.dim() > 0:
                for i in range(eos_indices.size(0) - 1, -1, -1):
                    ...
                    ret_scores[b_idx, res_k_idx] = scores[t][idx[0]].data[0]
                    ret_l[b_idx][res_k_idx] = t + 1

            # record the back tracked results
            output.append(current_output)
            h_t.append(current_hidden)
            ret_p.append(current_symbol)

            t -= 1

        # Sort and re-order again as the added ended sequences may change
        # the order (very unlikely)
        ret_scores, re_sorted_idx = ret_scores.topk(self.k)
        for b_idx in range(b):
            ret_l[b_idx] = [ret_l[b_idx][k_idx.item()] for k_idx in re_sorted_idx[b_idx, :]]
        ...
        ret_p = [step.index_select(0, re_sorted_idx).view(b, self.k, -1) for step in reversed(ret_p)]
        if lstm:
        ...
        ret_scores = ret_scores.data

        return output, h_t, h_n, ret_scores, ret_l, ret_p

    @staticmethod
    def mask_symbol_scores(score, idx, masking_score=-float('inf')):
        score[idx] = masking_score

    @staticmethod
    def mask(tensor, idx, dim=0, masking_score=-float('inf')):
        if len(idx.size()) > 0:
            indices = idx[:, 0]
